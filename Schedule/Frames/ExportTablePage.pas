...
unit ExportTablePage;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, StdCtrls, SCategory, ComCtrls, ToolWin, ImgList, Mask, PageDlg;

type
  TElementType = (etObject, etLessons);

  TfrmExportTablePage = class(TPageForm)
    lbElements: TListBox;
    cbSize: TComboBox;
...
constructor TfrmExportTablePage.Create(AOwner: TComponent);
begin
...
destructor TfrmExportTablePage.Destroy;
begin
  bmpPat.Free;
  bmpBuf.Free;
...
procedure TfrmExportTablePage.UpdateFrame;
begin
  ViewOrders;
  UpdateControls;
...
procedure TfrmExportTablePage.SetOptions(Value: TExportTableCategory);
begin
...
procedure TfrmExportTablePage.lbElementsClick(Sender: TObject);
var
  i: integer;
  fontsize: integer;
...
// отображение порядка
procedure TfrmExportTablePage.ViewOrders;
var
  i: integer;
  et: TElementType;
  s: string;
...
function TfrmExportTablePage.GetOrders: string;
var
  s: string;
  i: integer;
begin
...
procedure TfrmExportTablePage.OnElementBtnsClick(Sender: TObject);
...
procedure TfrmExportTablePage.cbSizeChange(Sender: TObject);
var
  size: integer;
begin
  Assert(Assigned(FOptions),
...
procedure TfrmExportTablePage.UpdateUpDownBtns;
var
  i: integer;
  bup,bdown: boolean;
begin
...
procedure TfrmExportTablePage.ComboDrawItem(Control: TWinControl; Index: Integer;
  Rect: TRect; State: TOwnerDrawState);
var
  c: TCanvas;
  oldbmp: TBitmap;
...
procedure TfrmExportTablePage.cbPatternChange(Sender: TObject);
var
  i: integer;
begin
  Assert(Assigned(FOptions),
...
procedure TfrmExportTablePage.chkMergeClick(Sender: TObject);
begin
  Assert(Assigned(FOptions),
...
procedure TfrmExportTablePage.cbOtherChange(Sender: TObject);
var
  n: integer;
begin
  Assert(Assigned(FOptions),
...
procedure TfrmExportTablePage.UpdateControls;
begin
...
procedure TfrmExportTablePage.btnDefaultClick(Sender: TObject);
begin
